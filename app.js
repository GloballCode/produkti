    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="field-group">
          ...
          <input type="number" id="edit-preco" value="${produto.preco}" min="0" step="0.01" />
        </div>
      </div>
...
window.salvarEdicaoProduto = async function(id) {
  ...
  if (!novoNome || novaQtd < 0 || novoPreco < 0) {
...
window.updateStats = function () {
  ...
  document.getElementById('stat-transacoes').textContent = state.sales.length;
};
...
window.renderSellPage = function () {
  ...
  state.products.forEach(p => {
    ...
    option.value = p.id;
    option.textContent = `${p.nome} - R$ ${p.preco.toFixed(2)} (${p.qtd} em estoque)`;
